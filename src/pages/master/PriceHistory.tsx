...
export default function PriceHistory() {
  return <MasterTable endpoint="/api/price-history" entityName="Price Entry" title="Price History" subtitle="Track product price changes over time" columns={[
    ...
    { key: 'product_name', label: 'Product', type: 'select', required: true, optionsEndpoint: '/api/products', onFieldChange: (form, key, value, opt) => ({ old_price: opt?.value ? (opt as any).current_price : '' }) },
    { key: 'old_price', label: 'Old Price', type: 'number', hideInForm: true, render: (r) => fmtMoney(r.old_price) },
