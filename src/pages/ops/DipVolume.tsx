...
export default function DipVolume() {
  ...
  const calc = () => {
    ...
    if (d <= points[0].dip_cm) { setResult(points[0].volume_liters); return; }
    if (d >= points[points.length - 1].dip_cm) { setResult(points[points.length - 1].volume_liters); return; }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="p-5">
          <div className="space-y-4">
            ...
            <Field label="Dip Reading (cm)" required><Input type="number" step="any" value={dip} onChange={(e) => setDip(e.target.value)} placeholder="e.g. 42.5" disabled={points.length < 2} /></Field>
        ...
        <Card>
          ...
          <div className="p-4 max-h-96 overflow-y-auto">
            {points.length === 0 ? <p className="text-sm text-slate-400 py-10 text-center">No calibration data</p> : (
              <table className="w-full text-sm"><thead className="text-xs text-slate-500"><tr><th className="text-left py-2">Dip (cm)</th><th className="text-right py-2">Volume (L)</th></tr></thead><tbody className="divide-y divide-slate-50">{points.map((p, i) => <tr key={i}><td className="py-1.5 text-slate-600">{fmtNum(p.dip_cm, 1)}</td><td className="py-1.5 text-right text-slate-700">{fmtNum(p.volume_liters, 0)}</td></tr>)}</tbody></table>
            )}
          </div>
        </Card>
      </div>
